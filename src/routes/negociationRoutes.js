...
} = require('../modules/negociation/testModule');
...
router.get('/', authorize, authorize(['USER', 'ADMIN']), getUserNegotiations);

// Detalhes de uma negociação
router.get('/:negotiationId', authenticate, authorize(['USER', 'ADMIN']), checkNegotiationExpiration, getNegotiationDetails);
...
// Adicionar mensagem
router.post('/:negotiationId/messages', authenticate, authorize(['USER', 'ADMIN']), checkNegotiationExpiration, addMessage);
